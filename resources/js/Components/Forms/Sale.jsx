...
export default (props) => {
    ...
    const { sale = null, genders, localities } = props

    const { zeroPad } = useLayout();

    const [month, setMonth] = useState(sale && sale.birthdate ? (new Date(sale.birthdate)).getMonth() : 0)
    const [year, setYear] = useState(sale && sale.birthdate ? (new Date(sale.birthdate)).getFullYear() : 1980)
    const [day, setDay] = useState(sale && sale.birthdate ? (new Date(sale.birthdate)).getDate() : 1)

    const { data, setData, post, patch, processing, errors, reset, transform } = useForm({
        id: sale ? sale.id : null,
        name: sale && sale.name ? sale.name : ``,
        lastname: sale && sale.lastname ? sale.lastname : ``,
        surname: sale && sale.surname ? sale.surname : ``,
        birthdate: sale && sale.birthdate ? sale.birthdate : '01.01.1980',
        tin: sale && sale.tin ? `${sale.tin}` : ``,
        email: sale && sale.email ? sale.email : ``,
        phone: sale && sale.phone ? sale.phone : ``,
        addon: sale && sale.addon ? sale.addon : ``,
        gender: sale && sale.gender ? sale.gender : `male`,
        locality_id: sale && sale.locality_id ? sale.locality_id : (localities.data.length ? localities.data[0].id : ``),
        branch_id: sale && sale.branch_id ? sale.branch_id : ``,
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        if (sale && sale.id)
            patch(route('admin.sales.update', {
                sale: sale.id
            }), {
                onSuccess: () => {
