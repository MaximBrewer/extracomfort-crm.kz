...
export default (props) => {
    ...
    const { accountant = null, genders, localities } = props

    const { zeroPad } = useLayout();

    const [month, setMonth] = useState(accountant && accountant.birthdate ? (new Date(accountant.birthdate)).getMonth() : 0)
    const [year, setYear] = useState(accountant && accountant.birthdate ? (new Date(accountant.birthdate)).getFullYear() : 1980)
    const [day, setDay] = useState(accountant && accountant.birthdate ? (new Date(accountant.birthdate)).getDate() : 1)

    const { data, setData, post, patch, processing, errors, reset, transform } = useForm({
        id: accountant ? accountant.id : null,
        name: accountant && accountant.name ? accountant.name : ``,
        lastname: accountant && accountant.lastname ? accountant.lastname : ``,
        surname: accountant && accountant.surname ? accountant.surname : ``,
        birthdate: accountant && accountant.birthdate ? accountant.birthdate : '01.01.1980',
        tin: accountant && accountant.tin ? `${accountant.tin}` : ``,
        email: accountant && accountant.email ? accountant.email : ``,
        phone: accountant && accountant.phone ? accountant.phone : ``,
        addon: accountant && accountant.addon ? accountant.addon : ``,
        gender: accountant && accountant.gender ? accountant.gender : `male`,
        locality_id: accountant && accountant.locality_id ? accountant.locality_id : (localities.data.length ? localities.data[0].id : ``),
        branch_id: accountant && accountant.branch_id ? accountant.branch_id : ``,
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        if (accountant && accountant.id)
            patch(route('admin.accountants.update', {
    ...
    return <form onSubmit={submit}>
        ...
        <div className={`flex space-x-8 items-center justify-end`}>
            <Link href={route('admin.accountants.index')}>
