...
export default (props) => {
    ...
    const { nurse = null, genders, localities } = props

    const { zeroPad } = useLayout();

    const [month, setMonth] = useState(nurse && nurse.birthdate ? (new Date(nurse.birthdate)).getMonth() : 0)
    const [year, setYear] = useState(nurse && nurse.birthdate ? (new Date(nurse.birthdate)).getFullYear() : 1980)
    const [day, setDay] = useState(nurse && nurse.birthdate ? (new Date(nurse.birthdate)).getDate() : 1)

    const { data, setData, post, patch, processing, errors, reset, transform } = useForm({
        id: nurse ? nurse.id : null,
        name: nurse && nurse.name ? nurse.name : ``,
        lastname: nurse && nurse.lastname ? nurse.lastname : ``,
        surname: nurse && nurse.surname ? nurse.surname : ``,
        birthdate: nurse && nurse.birthdate ? nurse.birthdate : '01.01.1980',
        tin: nurse && nurse.tin ? `${nurse.tin}` : ``,
        email: nurse && nurse.email ? nurse.email : ``,
        phone: nurse && nurse.phone ? nurse.phone : ``,
        addon: nurse && nurse.addon ? nurse.addon : ``,
        gender: nurse && nurse.gender ? nurse.gender : `male`,
        locality_id: nurse && nurse.locality_id ? nurse.locality_id : (localities.data.length ? localities.data[0].id : ``),
        branch_id: nurse && nurse.branch_id ? nurse.branch_id : ``,
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        if (nurse && nurse.id)
            patch(route('admin.nurses.update', {
                nurse: nurse.id
            }), {
                onSuccess: () => {
    ...
    return <form onSubmit={submit}>
        ...
        <div className={`flex space-x-8 items-center justify-end`}>
            <Link href={route('admin.nurses.index')}>
